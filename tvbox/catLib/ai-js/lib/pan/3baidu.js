...
// 全局调试开关
let DEBUG = globalThis.baidu_debug || 1;
...
async getRandsk(shareData) {
   ...
   // const postData = `pwd=${encodeURIComponent(shareData.pwd || '')}`;
    ...
    try {
        log('Randsk', `请求URL: ${verUrl}`);
        log('Randsk', `POST数据: ${formData}`);
        ...
        const response = await req(verUrl, {
            ...
            data: formData,
            postType: 'form'  // 设置postType为form
        });
        
        log('Randsk', `响应状态: ${response.status || response.code || 'unknown'}`);
        
        let result;
        if (response.content) {
            try {
                result = JSON.parse(response.content);
                log('Randsk', `响应内容: ${JSON.stringify(result)}`);
            } catch (e) {
                log('Randsk', `JSON解析失败: ${e.message}`);
                return null;
            }
        } else {
            log('Randsk', `无响应内容`);
        ...
        if (!result.error && result.errno === 0 && result.randsk) {
            ...
            log('Randsk', `获取randsk成功: ${randsk}`);
            return { randsk, cookie: independentCookie };
        } else {
            log('Randsk', `验证失败: errno=${result.errno}, err_msg=${result.err_msg || '未知错误'}`);
            return null;
        }
    } catch (error) {
        log('Randsk', `请求异常: ${error?.message || error}`);
        return null;
    }
}
    /**
